...
import com.gez.cookery.jiaoshou.adapter.RestaurantCommentListAdapter;
...
public class RestaurantCommentFragment extends SherlockFragment {
	
	// 列表对象
	private PullToRefreshListView lvListView;
	// 适配器
	private RestaurantCommentListAdapter lvListViewAdapter;
	// 列表尾部视图
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mRootView = inflater.inflate(R.layout.restaurant_list, null);
	...
	private void initListViewListView(View parentView, LayoutInflater inflater) {
		lvListViewAdapter = new RestaurantCommentListAdapter(getActivity(), lvListViewData);
		...
		lvListView = (PullToRefreshListView) parentView.findViewById(R.id.restaurant_listview);
	...
	private void loadLvListViewData(final int pageIndex,
		...
		// 加载数据
		RestClient.getBusinessCommentList(businessId, pageIndex, new IJsonModelArrayData() {
					public void onReturn(List<?> data) {
						...
						if (lvListViewAdapter.getCount() == 0) {
							...
							lvListView_foot_more.setText(R.string.load_restaurant_comment_empty);
						}
